...
const emit = defineEmits<{
  ...
  (e: "confirm", id: string): any;
}>();

// on click handler
const confirm = () => {
  emit("confirm", props.testid);
};

// on click handler
const discard = () => {
  emit("close", props.testid);
};
</script>

<template>
  <teleport to="body">
    <div v-show="isVisible" class="fixed z-10 inset-0 overflow-y-auto transform transition-all" role="dialog"
      aria-modal="true" :aria-labelledby="title">
      <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <!-- Background overlay -->
        <div class="fixed inset-0 bg-gray-700 bg-opacity-50" aria-hidden="true" />
        ...
        <div id="modal-panel"
          ...
          <div>
            <div v-if="icon" id="icon-container"
